...
onMounted(() => {
    if (route.query.only_friday == 'true') {
        static_config.value.only_friday = true;
        static_config.value.meal = false;
    } else if (route.query.meal == 'true') {
        static_config.value.only_friday = false;
        static_config.value.meal = true;
    } else {
        static_config.value.only_friday = false;
        static_config.value.meal = false;
    }
...
async function get_ticket() {
    ...
    const api = await api_post(config.endpoint_ticket, { method: 'get_ticket', parameters: { form: formVal.value, static_cfg: static_config.value, price: money.value, lang: i18n.global.t('lang_code') } });
    if (config.debug) {
...
<template>
    ...
    <div class="intro-page-wraper">
                            ...
                            <div id="ticket_birthday" class="flex-wrap gap-2 w-full">
                                <FloatLabel variant="on" class="w-full">
                                    ...
                                    <DatePicker
                                        ...
                                        :invalid="touched.birthday && !valid.birthday"
                                        dateFormat="yy-mm-dd"
                                        :locale="locale"
                                        @update:modelValue="(val) => (formVal.birthday = val ? val.toISOString().split('T')[0] : '')"
                                    ></DatePicker>
                                </FloatLabel>
                            </div>
                        </div>
        ...
        <div v-if="EmailVerified">
            ...
            <br />
            <div class="my-right"><Button :label="$t('ticket_get_ticket')" style="width: 150px" @click="toggle1" :disabled="btnLoading" :loading="btnLoading"></Button></div>
