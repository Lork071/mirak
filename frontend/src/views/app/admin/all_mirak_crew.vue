...
async function load_all_participant() {
    const api = await api_post(config.endpoint_volunteer, { method: 'read_all_mirak_crew', parameters: {} });
    if (config.debug) {
        console.log('API [read_all_mirak_crew]: ');
        console.log(api);
    }
    if (api.result) {
...
function open_participant(id_ticket) {
    window.open('/app/mirak-crew-person?id=' + id_ticket, '_blank');
}
...
<template>
    <div className="card">
        <div class="font-semibold text-xl mb-4 mt-8">{{ $t('all_participant_unprocessed') }}</div>
        <DataTable
            ref="dtUnprocessed"
            :value="crew_persons?.filter((p) => p.processed === 0)"
            dataKey="id"
            :paginator="true"
            :rows="10"
            :filters="filters"
    ...
    <div className="card">
        <div class="font-semibold text-xl mb-4">{{ $t('all_participant_processed') }}</div>
        <DataTable
            ref="dtProcessed"
            :value="crew_persons?.filter((p) => p.processed === 1)"
            dataKey="id"
            :paginator="true"
            :rows="10"
            :filters="filters"
